...
vendor/bin/doctrine-dbal dbal:import $DEMODIR/current/*

techo "migrate latest database definitation. (exe no-interaction)"
vendor/bin/doctrine-dbal dbal:migrate $DEMODIR/latest/* --no-interaction -v -k

techo "confirm no diff"
vendor/bin/doctrine-dbal dbal:migrate $DEMODIR/latest/* --check
